...
export default function CreateListingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if ((formData.listingType === 'SWAP' || formData.listingType === 'BOTH') && formData.swapTerms.trim().length > 0 && formData.swapTerms.trim().length < 3) {
      toast.error('Swap terms must be between 3 and 500 characters'); return;
    }
    if ((formData.listingType === 'SWAP' || formData.listingType === 'BOTH') && formData.swapTerms.trim().length > 500) {
      toast.error('Swap terms must be between 3 and 500 characters'); return;
    }
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {(formData.listingType === 'SWAP' || formData.listingType === 'BOTH') && (
                <div>
                  ...
                  />
                  <p className="text-xs text-muted-foreground mt-1">
                    {formData.swapTerms.length}/500 characters
                  </p>
                </div>
              )}
