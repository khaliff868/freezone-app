...
import {
  ...
  MousePointerClick, ArrowLeft, Pencil, X, Check, Upload, Loader2,
} from 'lucide-react';
...
export default function UserBannersPage() {
  ...
  const resetForm = () => {
    setForm({ title: '', imageUrl: '', linkUrl: '', placement: 'homepage_top_middle', startDate: '', endDate: '' });
    setEditingId(null);
    setShowForm(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(editingId ? 'Banner updated!' : 'Banner created! Please submit payment proof below to activate.');
      resetForm();
      loadBanners();
      if (!editingId) {
  ...
  const handleUploadPaymentProof = async (bannerId: string, file: File) => {
    try {
      ...
      const submitRes = await fetch('/api/banners/my', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: bannerId, action: 'submit_payment', paymentMethod: 'BANK_DEPOSIT', paymentProofUrl: publicUrl, paymentReference: reference }),
      });
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                <Megaphone className="w-6 h-6 text-tropical-purple" />My Banner Ads
              </h1>
        ...
        <div className="bg-gradient-to-r from-tropical-purple/10 to-tropical-pink/10 dark:from-tropical-purple/20 dark:to-tropical-pink/20 rounded-2xl p-4 mb-6 border border-tropical-purple/20">
          <p className="text-sm text-gray-700 dark:text-gray-300">
            ...
            <span className="font-medium">1400×263px</span>. After creating an ad, submit payment proof for admin verification.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
            ...
            <button onClick={() => { resetForm(); setShowForm(true); }}
              ...
              <Plus className="w-5 h-5" />Create First Ad
            </button>
          </div>
        ) : (
          <div className="space-y-4">
            {banners.map((banner) => (
              <div key={banner.id}
                className={`bg-white rounded-xl shadow-md overflow-hidden border-2 transition-colors ${banner.active ? 'border-transparent hover:border-tropical-purple/20' : 'border-gray-200'}`}>
                <div className="flex flex-col md:flex-row">
                  ...
                  <div className="flex-1 p-4">
                    ...
                    {banner.endsAt && (
                      <p className="text-xs text-gray-400 mt-2">Expires: {new Date(banner.endsAt).toLocaleDateString()}</p>
                    )}
                    ...
                    {banner.status === 'PENDING_PAYMENT' && (
                      <div className="mt-3 p-3 bg-orange-50 rounded-lg border border-orange-200">
                        ...
                        <p className="text-xs text-orange-600 mt-1">Upload payment proof to activate this banner ad for 30 days.</p>
                      </div>
                    )}
                    ...
                    {banner.status === 'PENDING_VERIFICATION' && (
                      <div className="mt-3 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                        ...
                        <p className="text-xs text-yellow-700 mt-1">Your banner ad is awaiting admin verification. Activation starts after approval and runs for 30 days.</p>
                      </div>
                    )}
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-3">
                        ...
                        <>
                          ...
                          <label className="flex items-center gap-1 px-3 py-1.5 text-sm bg-gradient-to-r from-caribbean-green to-tropical-lime text-white font-medium rounded-lg hover:opacity-90 transition cursor-pointer">
                            <input type="file" accept="image/*,.pdf" className="hidden"
                              ...
                              onChange={(e) => { const file = e.target.files?.[0]; if (file) handleUploadPaymentProof(banner.id, file); }} />
                      ...
                      {(banner.status === 'ACTIVE' || banner.status === 'EXPIRED') && (
                        <button onClick={async () => {
                          try {
                            const res = await fetch('/api/banners/my', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: banner.id, action: 'renew' }) });
                            if (!res.ok) throw new Error('Failed');
                            toast.success('Renewal initiated. Please submit payment proof.');
                            loadBanners();
                          } catch { toast.error('Failed to initiate renewal'); }
                    ...
                    {(banner.status === 'ACTIVE' || banner.status === 'EXPIRED') && (
                      <p className="text-xs text-gray-500 mt-2">Submit payment proof to activate or renew this banner ad for 30 days.</p>
                    )}
                  </div>
                </div>
              </div>
