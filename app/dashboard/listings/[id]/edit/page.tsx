...
export default function EditListingPage() {
  ...
  const loadListing = async () => {
    try {
      ...
      // Check ownership (client-side check, server also enforces)
      if (listingData.user.id !== session?.user?.id && session?.user?.role !== 'ADMIN') {
  ...
  // Auto-sync between Free Items category and FREE listing type
  useEffect(() => {
    // If Free Items category is selected, auto-set listing type to FREE and price to 0
    if (formData.category === 'Free Items' && formData.listingType !== 'FREE') {
      setFormData(prev => ({ ...prev, listingType: 'FREE', price: '0' }));
    }
    // If FREE listing type is selected, auto-set category to Free Items and price to 0
    if (formData.listingType === 'FREE' && formData.category !== 'Free Items') {
      setFormData(prev => ({ ...prev, category: 'Free Items', price: '0' }));
    }
    // If FREE listing type is selected, ensure price is 0
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (const file of filesToUpload) {
        ...
        // For demo, use placeholder (same as create page)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Only include swapTerms if swapping
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                      ...
                      <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1">
                        {index > 0 && (
                          <button
                            ...
                            className="p-1 bg-white/80 text-gray-800 rounded hover:bg-white"
                            title="Move left"
                          >
                            ←
                          </button>
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  First image will be the main listing photo. Use arrows to reorder.
                </p>
              </div>
              ...
              <div>
                ...
                <Input
                  id="title"
                  placeholder="What are you selling/swapping?"
              ...
              {/* Price (if selling) - Hidden for FREE listings */}
              ...
              {formData.listingType === 'FREE' && (
                <div className="p-4 bg-caribbean-green/10 border border-caribbean-green/30 rounded-xl">
                  ...
                  <p className="text-sm text-gray-600">
                    This item is free. No payment is required.
                  </p>
                  <p className="text-xs text-gray-500 mt-1">
                    Price: <span className="font-semibold">0 TTD</span> • Expires after 30 days • Admin approval required
                  </p>
                </div>
              )}
              ...
              {(formData.listingType === 'SWAP' || formData.listingType === 'BOTH') && (
                <div>
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      setFormData((prev) => ({ ...prev, swapTerms: e.target.value }))
                    }
                    rows={2}
                  />
                </div>
